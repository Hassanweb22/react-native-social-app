...
const IndividualComments = ({ comment, userID, commentTime }) => {
    ...
    useEffect(() => {
        ...
        console.log("comCreatedat", date)
        setComTime(date)
    }, [])

    const { photoURL, firstname, lastname } = commenttator


    const handleDelete = () => {
        const deleteComment = async () => {
            await database().ref(`users/${userID}/posts/${comment.postID}/comments/${comment.comKey}`).remove().then(() => {
                Toast.show({
                    type: "success",
                    position: "top",
                    topOffset: 40,
    ...
    return (
        <View style={{ padding: 10, paddingBottom: 0, marginBottom: 0 }} onTouchEnd={() => {
            setComTime(moment(comment.createdAt).fromNow())
        }}>
            <Card style={{ borderColor: colors.green, borderWidth: 1, borderRadius: 10, padding: 4 }}
                onTouchEnd={() => actions && setActions(false)}
            >
                <CardItem bordered>
                    <Left>
                        <View>
