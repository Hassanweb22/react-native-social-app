import React, { useState, useEffect, useCallback } from 'react';
import { View, ScrollView, Text, Button, StyleSheet, SafeAreaView, Dimensions, StatusBar, TouchableOpacity } from 'react-native';
import { Bubble, GiftedChat, Send, InputToolbar, InputToolbarProps } from 'react-native-gifted-chat';
import Ionicons from 'react-native-vector-icons/Ionicons';
import FontAwesome from 'react-native-vector-icons/FontAwesome';
import colors from '../../colors/colors';
import { useSelector } from 'react-redux';
import firebase from "@react-native-firebase/app"
import firestore from "@react-native-firebase/firestore"
import database from "@react-native-firebase/database"
import moment from 'moment';
import { CardItem, Form, Icon, Input, Item } from 'native-base';

const ChatScreen = (props) => {
    const { userKey, user, currentUser } = props.route.params


    const temURI = "data:image/png;base64,"
    const currentUserUID = useSelector(state => state.todo.loginUser.uid);
    const isDark = useSelector(state => state.todo.dark)

    console.log(currentUser, "currentUser")
    const [messages, setMessages] = useState([]);

    const getMessages = async () => {

        const combineID = userKey > currentUserUID ? userKey + "-" + currentUserUID : currentUserUID + "-" + userKey
        await firestore().collection("chatrooms")
            .doc(combineID)
            .collection("messages")
            .orderBy("createdAt", "desc")
            .onSnapshot(snap => {
                let arr = []
                snap.docs.map(doc => {
                    arr.push({
                        ...doc.data(),
                        _id: doc.id,
                        createdAt: doc.data().createdAt.toDate(),
                    })
                })
                // console.log('messages', arr)
                setMessages(arr);
            });

    }

    useEffect(() => {
        return getMessages()
    }, []);

    const onSend = async (messageArr) => {
        console.log(messageArr)
        const msg = messageArr[0]
        const myMesg = {
            ...msg,
            sentTo: userKey,
            sentBy: currentUserUID,
        }
        // console.log("mymsg", myMesg)
        const combineID = userKey > currentUserUID ? userKey + "-" + currentUserUID : currentUserUID + "-" + userKey
        await firestore().collection("chatrooms").doc(combineID).collection("messages").add(myMesg).then(_ => console.log("message sent"))
    };

    const renderSend = (props) => {
        return (
            <Send {...props}>
                <View>
                    <Ionicons
                        name="send"
                        style={{ marginBottom: 5, marginRight: 5 }}
                        size={32}
                        color={colors.green}
                    />
                </View>
            </Send>
        );
    };

    const renderBubble = (props) => {
        return (
            <Bubble
                {...props}
                wrapperStyle={{
                    right: {
                        backgroundColor: colors.green,
                    },
                    left: {
                        backgroundColor: "#ECEFF1",
                    },
                }}
                textStyle={{
                    right: {
                        color: '#fff',
                    },
                }}
            />
        );
    };

    const renderInput = (props) => {

        return (
            <View style={[{ overflow: "hidden" }]}>
                <Form>
                    <Item style={{ backgroundColor: "green" }} rounded placeholder placeholderLabel="Comment">
                        <Input {...InputToolbarProps} style={{ backgroundColor: "#fff" }} {...props} placeholder="Comment Here..." numberOfLines={5}
                        // value={state.comment} onChangeText={(value) => onChangeText(value, "comment")}
                        />
                        {/* <TouchableOpacity style={{ borderWidth: 0, paddingHorizontal: 10 }}>
                            <Text>
                                <Ionicons name="send" color={"grey"} size={30} />
                            </Text>
                        </TouchableOpacity> */}
                    </Item>
                </Form >
            </View>
        )
    }

    const scrollToBottomComponent = () => {
        return (
            <View>
                <FontAwesome name='angle-double-down' size={22} color='#000' />
            </View>
        );
    }

    return (
        <SafeAreaView removeClippedSubviews style={{ flex: 1, height: Dimensions.get('screen').height - StatusBar.currentHeight, backgroundColor: isDark ? colors.dark : "#fff" }}>
            <StatusBar barStyle="light-content" backgroundColor={colors.green} />
            <GiftedChat
                messages={messages}
                onSend={(messages) => onSend(messages)}
                user={{
                    _id: currentUserUID,
                    avatar: currentUser.photoURL ? currentUser.photoURL : temURI
                }}
                renderBubble={renderBubble}
                // renderInputToolbar={renderInput}
                // showAvatarForEveryMessage
                renderAvatar={null}
                renderSend={renderSend}
                scrollToBottom
                scrollToBottomComponent={scrollToBottomComponent}
            />
        </SafeAreaView>
    );
};

export default ChatScreen;

const styles = StyleSheet.create({
    container: {
        flex: 1,
        alignItems: 'center',
        justifyContent: 'center',
    },
    addComment: {
        flexDirection: "row",
        position: "relative",
        justifyContent: "space-between",
        alignItems: "center",
        borderWidth: 1,
        borderColor: colors.green,
        borderRadius: 30,
        marginVertical: 15,
        marginHorizontal: 10,
        elevation: 10,
        shadowOffset: {
            width: 0,
            height: -2,
        },
        shadowOpacity: 0.25,
        shadowRadius: 3.84,
        // shadowColor: colors.green
    }
});
