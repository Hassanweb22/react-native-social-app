...
import { View, ScrollView, Text, Button, StyleSheet, SafeAreaView, Dimensions, StatusBar } from 'react-native';
...
const ChatScreen = (props) => {
    ...
    //         // tabBarLabel: "CHAAT"
    //     });
    //     // console.log('propsChat: ', navigation);
    //     return () =>
    //         parent.setOptions({
    //             tabBarVisible: true
    //         });
    ...
    const getMessages = async () => {
        // const userChatwala = database().ref(`users/`).child().on("value", snap => {
    ...
    return (
        <View removeClippedSubviews style={{ flex: 1, height: Dimensions.get('screen').height - StatusBar.currentHeight, backgroundColor: isDark ? colors.dark : "#fff" }}>
            <GiftedChat
                messages={messages}
